...
export const TAB_BAR_WEB_PADDING_BOTTOM = 6;
...
export function getTabBarPaddingBottom(bottomInset: number): number {
  return Platform.OS === 'web'
    ? TAB_BAR_WEB_PADDING_BOTTOM
    : TAB_BAR_PADDING_BOTTOM + bottomInset;
}

export function getTabBarHeight(bottomInset: number): number {
  return TAB_BAR_PADDING_TOP + TAB_BAR_CONTENT_HEIGHT + getTabBarPaddingBottom(bottomInset);
}
